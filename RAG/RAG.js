...
try {
  ...
  try {
    // Fallback to direct dist path if package exports don't expose the subpath
    RecursiveCharacterTextSplitter = require("langchain/dist/text_splitter").RecursiveCharacterTextSplitter;
  } catch (e2) {
    try {
      // Try alternate scoped package (if installed separately)
      RecursiveCharacterTextSplitter = require("@langchain/text_splitter").RecursiveCharacterTextSplitter;
    } catch (e3) {
      // Provide a minimal local fallback implementation compatible with usage below.
      // This simple splitter chunks by characters with overlap and keeps metadata.
      ...
      console.warn(
        "Using local RecursiveCharacterTextSplitter fallback — consider installing a langchain version that exposes 'text_splitter' exports for full functionality."
      );
    }
  }
}
...
try {
  ...
  // Provide a lightweight local MemoryVectorStore fallback that uses simple
  // text-match retrieval if the langchain package doesn't export the vectorstore.
  class LocalMemoryVectorStore {
    ...
    static async fromDocuments(docs, embeddings) {
      ...
      return new LocalMemoryVectorStore(docs.map((d) => ({ pageContent: d.pageContent || d.content || "", metadata: d.metadata || {} })));
    }
...
classObj.generateOutput("Background garbage collection in C# and how it works?");
